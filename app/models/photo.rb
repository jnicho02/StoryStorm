...
class Photo < ApplicationRecord
  ...
  def match
    if !plaque_id
      nearest_plaques&.each do |nearest|
        if !plaque_id && (nearest.inscription.downcase.include?(subject.downcase) || (subject.match(/([\w\s]*),/) && nearest.inscription.downcase.include?(subject.match(/([\w\s]*),/)[1].downcase)) )
  ...
  def uri
    "http://storystorm.herokuapp.com" + Rails.application.routes.url_helpers.photo_path(self, format: :json)
  end
