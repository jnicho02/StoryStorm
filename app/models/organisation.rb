...
class Organisation < ApplicationRecord
  ...
  def uri
    "http://storystorm.herokuapp.com#{Rails.application.routes.url_helpers.organisation_path(self.slug, :format=>:json)}"
  end

  def plaques_uri
    "http://storystorm.herokuapp.com#{Rails.application.routes.url_helpers.organisation_plaques_path(self.slug, format: :geojson)}"
  end
